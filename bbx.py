...
class LofasmFile(object):
    ...
    def __init__(
        self, lofasm_file, header={"metadata": {}}, verbose=False, mode="read", gz=None
    ):
        ...
        if mode in ["read"] and gz == None:
            try:
                ...
                if hasattr(e, "message") and e.message == "Not a gzipped file":
                    gz = False
                else:
                    raise IOError(str(e))
        elif mode == "write":
        ...
        if mode in ["read"]:
            self._fp = (
                gzip.open(self.fpath, self._fmode)
                if gz
                else open(self.fpath, self._fmode)
            )
            self._load_header()
        elif mode == "write":
    ...
    def add_data(self, data):
        ...
        if self._new_file:
            ...
            self.data = np.zeros(
                N, dtype=np.complex128 if np.iscomplexobj(data) else np.float64
            )
            ...
            if str(dim2_bins) != str(self.dim2_len):
                raise ValueError(
                    "new data length for dim2 must match the existing data!"
                )
    ...
    def read_data(self, N=None):
        ...
        # Move file pointer to start of data block if not already there
        # (Assume header has been read and pointer is at start of data)

        # Calculate total number of elements
        nrows = int(N)
        ncols = int(self.dim2_len)
        dtype = np.float64 if not self.iscplx else np.complex128
        nbits = self.nbits
        ...
        if not self.iscplx:
            ...
            nbytes = total_elems * (nbits // 8)
            raw = self._fp.read(nbytes)
            arr = np.frombuffer(raw, dtype=np.float64)

            # Check if the array size matches expected dimensions
            expected_size = nrows * ncols
            actual_size = arr.size

            if actual_size != expected_size:
                # Try to infer the correct dimensions
                if actual_size % ncols == 0:
                    # Adjust nrows to match the actual data size
                    nrows = actual_size // ncols
                    if self.debug:
                        print(
                            f"Warning: Array size mismatch. Expected {expected_size} elements, got {actual_size}. Adjusting nrows from {int(N)} to {nrows}"
                        )
                elif actual_size % nrows == 0:
                    # Adjust ncols to match the actual data size
                    ncols = actual_size // nrows
                    if self.debug:
                        print(
                            f"Warning: Array size mismatch. Expected {expected_size} elements, got {actual_size}. Adjusting ncols from {int(self.dim2_len)} to {ncols}"
                        )
                else:
                    # If neither dimension divides evenly, try to find the best fit
                    # Find the closest rectangular shape
                    best_nrows = int(np.sqrt(actual_size))
                    while actual_size % best_nrows != 0 and best_nrows > 1:
                        best_nrows -= 1

                    if best_nrows > 1:
                        nrows = best_nrows
                        ncols = actual_size // best_nrows
                        if self.debug:
                            print(
                                f"Warning: Array size mismatch. Expected {expected_size} elements, got {actual_size}. Using best fit: {nrows}x{ncols}"
                            )
                    else:
                        # If we can't find a good fit, flatten to 1D
                        nrows = actual_size
                        ncols = 1
                        if self.debug:
                            print(
                                f"Warning: Array size mismatch. Expected {expected_size} elements, got {actual_size}. Flattening to 1D array of size {actual_size}"
                            )

            try:
                self.data = arr.reshape((nrows, ncols))
            except ValueError as e:
                # If reshape still fails, try to handle it more gracefully
                if self.debug:
                    print(f"Warning: Reshape failed for real data: {e}")
                    print(f"Array size: {arr.size}, attempting to reshape as 1D array")
            ...
            # Complex data
            total_elems = nrows * ncols * 2  # real+imag
            nbytes = total_elems * (nbits // 8)
            raw = self._fp.read(nbytes)
            arr = np.frombuffer(raw, dtype=np.float64)

            # Check if the array size matches expected dimensions
            expected_size = nrows * ncols * 2
            actual_size = arr.size

            if actual_size != expected_size:
                # For complex data, we need an even number of elements
                if actual_size % 2 != 0:
                    # Remove the last element if it's odd
                    arr = arr[:-1]
                    actual_size = arr.size

                # Try to infer the correct dimensions
                if actual_size % (2 * ncols) == 0:
                    # Adjust nrows to match the actual data size
                    nrows = actual_size // (2 * ncols)
                    if self.debug:
                        print(
                            f"Warning: Complex array size mismatch. Expected {expected_size} elements, got {actual_size}. Adjusting nrows from {int(N)} to {nrows}"
                        )
                elif actual_size % (2 * nrows) == 0:
                    # Adjust ncols to match the actual data size
                    ncols = actual_size // (2 * nrows)
                    if self.debug:
                        print(
                            f"Warning: Complex array size mismatch. Expected {expected_size} elements, got {actual_size}. Adjusting ncols from {int(self.dim2_len)} to {ncols}"
                        )
                else:
                    # If neither dimension divides evenly, try to find the best fit
                    # For complex data, we need to account for the 2x factor
                    best_nrows = int(np.sqrt(actual_size // 2))
                    while (actual_size // 2) % best_nrows != 0 and best_nrows > 1:
                        best_nrows -= 1

                    if best_nrows > 1:
                        nrows = best_nrows
                        ncols = (actual_size // 2) // best_nrows
                        if self.debug:
                            print(
                                f"Warning: Complex array size mismatch. Expected {expected_size} elements, got {actual_size}. Using best fit: {nrows}x{ncols}"
                            )
                    else:
                        # If we can't find a good fit, flatten to 1D
                        nrows = actual_size // 2
                        ...
                        if self.debug:
                            print(
                                f"Warning: Complex array size mismatch. Expected {expected_size} elements, got {actual_size}. Flattening to 1D array of size {actual_size//2}"
                            )

            try:
                arr = arr.reshape((nrows, ncols, 2))
                self.data = arr[..., 0] + 1j * arr[..., 1]
            except ValueError as e:
                # If reshape still fails, try to handle it more gracefully
                if self.debug:
                    print(f"Warning: Reshape failed for complex data: {e}")
                    print(
                        f"Array size: {arr.size}, attempting to reshape as 1D complex array"
                    )

                # Reshape as 1D complex array
                if arr.size % 2 == 0:
                    arr = arr.reshape(-1, 2)
                    self.data = arr[:, 0] + 1j * arr[:, 1]
                else:
                    # Remove the last element if odd
    ...
    def write(self):
        ...
        if missing_keys:
            errmsg = "header missing required fields: {}".format(
                ", ".join(missing_keys)
            )
    ...
    def _load_header(self):
        try:
            ...
            if isinstance(fsig, bytes):
                fsig = fsig.decode("utf-8")
            if fsig.startswith("%"):
                fsig = fsig.strip("%")
            elif self.gz:
                raise IOError(
                    "Unable to parse file. Unrecognizable file signature. Are you sure compression should be turned on?"
                )
            else:
            ...
            if (
                self.gz
                and hasattr(e, "strerror")
                and e.strerror == "Not a gzipped file"
            ):
                raise IOError(
                    "Compression parameter is True but input file is not a gzipped file"
                )
            else:
                raise IOError(str(e))
        ...
        if fsig not in SUPPORTED_FILE_SIGNATURES:
            raise NotImplementedError(
                "{} is not a supported LoFASM file signature".format(fsig)
            )
        ...
        if missing_comment_fields:
            raise RuntimeError(
                "Missing required comment fields for {} header type: {}".format(
