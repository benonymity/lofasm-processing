...
def convert_bbx_to_npy():
    ...
    filtered_files = [f for f in all_bbx_files if any(f.name.endswith(suffix) for suffix in VALID_BBX_SUFFIXES)]
...
def plot_npy_files():
    ...
    for npy_file in npy_files:
        ...
        plt.yticks([602.25, 1204.5, 1806.75, 2409, 3011.25],
                   ['50', '100', '150', '200', '250'])
...
def get_remote_subfolders(remote_base):
    ...
    folders = [f.rstrip('/').strip() for f in folders if f.rstrip('/').lower() != "processing"]
    return folders

def organize_files_by_date(files, local_folder, ext, suffix="", create_folder_func=None):
    date_folders = {}
    for file in files:
        name = file.name
        if len(name) < 8:
...
def main():
    ...
    for subfolder in subfolders:
        ...
        if len(subfolder) == 8 and subfolder.isdigit() and subfolder not in TARGET_DATES:
            continue
        ...
        for f in filtered_files:
            ...
            files_to_process_by_date.setdefault(date_prefix, []).append( (remote_subfolder, f) )
    ...
    for date_prefix in TARGET_DATES:
        ...
        for remote_subfolder, filename in files_for_date:
            local_file = rclone_download_file(remote_subfolder, filename, LOCAL_LOFASM_DIR)
        ...
        print(f"✅ Finished processing date {date_prefix}. Files done: {processed_count}, Remaining: {remaining}")
